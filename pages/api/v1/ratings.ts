...
export default withRateLimit(async (req: AuthenticatedRequest, res: NextApiResponse) => {
  if (req.method === 'POST') {
    if (req.user.role !== 'employer') {
      return res.status(403).json({ error: 'Only employers can rate', code: 'FORBIDDEN' })
    }
    ...
    if (!application_id || !score || score < 1 || score > 5) {
      return res.status(400).json({ error: 'Valid application_id and score (1-5) required', code: 'VALIDATION_ERROR' })
    }
    ...
    await logActivity({ userId: req.user.userId, apiKeyId: req.apiKeyId, action: 'rate_agent', resourceType: 'rating', resourceId: rating.id })
  ...
  if (req.method === 'GET') {
    ...
    if (!seeker_id) {
      return res.status(400).json({ error: 'seeker_id query parameter required', code: 'VALIDATION_ERROR' })
    }
